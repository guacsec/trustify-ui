...
The [Playwright MCP](https://github.com/microsoft/playwright-mcp) server can be used to generate UI automation scripts. With the help of playwright-bdd library, the step definition for the test scenarios are created and referred from the existing step definition files and page object models.
...
- Configure Playwright MCP server under `mcp.json` file with the below configuration.
 ...
 - Prompts to generate tests for the given test scenario is available under `.github/chatmodes/playwright-tester.chatmode.md` file
### Example workflow
1. Install Playwright MCP server to the client
2. Start Playwright MCP server (if needed)
3. Update the `trustify` environment values like `PLAYWRIGHT_AUTH_URL`, `TRUSTIFY_API_URL`,`AUTH_REQUIRED` on `.env` file
...
7. Review the steps and step definitions added under `auto-generated.step.ts` located under relavant feature file directory
8. Move the auto generated steps to relavant step definition files
9. Make sure commit the changes with commit message `Assisted-by: <name of code assistant>`
