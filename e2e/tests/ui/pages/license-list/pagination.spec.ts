...
test.describe("Pagination validations", { tag: "@tier1" }, () => {
  ...
  test("First page state - Previous/First buttons disabled, Next enabled", async ({
    page,
  }) => {
  ...
  test("Next page navigation - Navigate to page 2, verify Previous/First enabled", async ({
    page,
  }) => {
    const listPage = await LicenseListPage.build(page);
    const pagination = await listPage.getPagination();
  ...
  test("Items per page - 10 items", async ({ page }) => {
    const listPage = await LicenseListPage.build(page);
  ...
  test("Items per page - 20 items", async ({ page }) => {
    const listPage = await LicenseListPage.build(page);
