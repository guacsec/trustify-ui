...
export const VulnerabilitiesBySbom: React.FC<VulnerabilitiesBySbomProps> = ({
  ...
  const purls = useMemo(() => {
    ...
    for (const vuln of vulnerabilities) {
      for (const [, analysis] of vuln.purls) {
        if (!analysis.isOrphan) {
          purlSet.add(analysis.purlSummary.purl);
        }
      }
    }
    ...
    filterCategories: [
      {
        ...
        matcher: (_filterValue, item) => {
          const cveId = item.vulnerability?.identifier ?? "";
          return (
            (recommendedVersionByCve.get(cveId)?.length ?? 0) > 0 ||
            (item.vulnerability?.fixed_versions?.length ?? 0) > 0
          );
        },
      },
    ],
                    ...
                    >
                      ...
                      >
                        {(() => {
                          ...
                          const fixedVersions =
                            item.vulnerability.fixed_versions;
