...
function buildOptionMap(options: SelectOption[], map = new Map<string, SelectOption>()): Map<string, SelectOption> {
...
export const MenuWithDrilldown: React.FunctionComponent<MenuWithDrilldownProps> = ({ options, onSelect, onInputChange }) => {
            ...
            drilldownMenu={
                <DrilldownMenu id={`drilldown-menu-${option.id}`}>
                    ...
                    {option.children!.map((child) => (
                        <DrilldownMenuItem key={child.id} option={child} onSelect={onSelect} />
                    ))}
                </DrilldownMenu>
            }
        >
